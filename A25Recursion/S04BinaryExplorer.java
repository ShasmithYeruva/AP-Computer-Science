/* TODO: 04
The Binary Search algorithm works by testing a mid-point, then eliminating 
half of the list.

In this exercise, you are going to take our binary search algorithm and 
add print statements so that you can track how the search executes.

Inside of the recursive binary search function, add print statements to 
print out the starting, ending, and midpoint values each time.

Then as you test a value, print out the results, either too high, too low, 
or a match.
...
====
 */
public class S04BinaryExplorer {
	...
	public static int binaryRec(int[] array, int target, int begin, int end) {
		...
		if (begin <= end)
		{
			...
			if (target < array[mid]) {
				System.out.println("Target is lower than mid!");
