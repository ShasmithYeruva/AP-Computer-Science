/* TODO: 05
In this exercise you will compare binary search and linear (or sequential) 
search.

We have included the algorithms for you, however right now the method returns 
the index value where the number is found.

What you need to do in this problem is modify each method to instead return 
the number of times each goes through the loop.

Then you can test out the results on lists of different sizes. We have 
	...
	public static int linearSearch(int[] array, int number)
	{
		...
		for (int i = 0; i < array.length; i++)
		{
			...
			if (array[i] == number)
			{
				...
				return count; // the method returns as soon as the number is found
			}
		}
		return count; // the code will get here if the number isn't found
	}
}